...
The hardcoded IDs below are correct for this repo as of 2026-04-09 00:49:02 UTC. If they ever change, or if you're working in a fork, re-run these queries to get fresh values:

```bash
# Repository node ID
gh api graphql -f query='{ repository(owner: "pbulsink", name: "naturalstattrick") { id } }'

# Available issue type IDs
gh api graphql -f query='{ repository(owner: "pbulsink", name: "naturalstattrick") { issueTypes(first: 20) { nodes { id name description } } } }'
```
